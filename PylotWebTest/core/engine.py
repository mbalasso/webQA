            ...
            for req in self.msg_queue:
                for repeat in range(req.repeat):
                    if self.running:
                        ...
                        #login case, use a different user for each agent                        
                        #default username testuserX, X= agent.id                         
                        if req.login:
                            req.body = str(req.body).replace( '__ac_name=testuser', '__ac_name=testuser%s' % str(self.id) ) 
                            #print "\n\nafter: %s \n\n"  % req.body
                        ...
                        if not req.verify == '':
                            if not re.search(req.verify, content, re.DOTALL): 
                                #print req.verify
                                outfile = open('testverify_agent%s.txt' % self.id,'w')
