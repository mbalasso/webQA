...
def generate_results(dir, test_name):
    ...
    fstat = open(dir + '/statistics.txt', 'w')
    fstat.write(str(stats_dict))
    fstat.close()
    ...
    reportwriter.write_error_log(fh, merged_error_log)
...
def best_and_worst_requests(merged_log):  # get the fastest/slowest urls
    ...
    worst_times = {}
    for x in url_times:
        #if url_times[x] in raw_times[:3]:  
        if url_times[x] in raw_times:  # take the top 3
           best_times[x] = url_times[x]
    for x in url_times:
        #if url_times[x] in raw_times[-3:]:  # take the bottom 3
        if url_times[x] in raw_times: 
